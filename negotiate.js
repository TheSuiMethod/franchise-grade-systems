...
module.exports = async function handler(req, res) {
  ...
  // Stripe payment verification — negotiation simulator is part of Decision Engine ($297) and Bundle ($897)
  const NEGOTIATE_ACCESS_PRICE_IDS = [
    'price_1T4XuECtSsWNQjR9dvj2lksh', // Decision Engine ($297)
    'price_1T2incCtSsWNQjR95AMv1AeX',  // Bundle ($897)
  ];
  ...
  try {
    ...
    const hasAccess = lineItems.some(item =>
      NEGOTIATE_ACCESS_PRICE_IDS.includes(item.price?.id)
    );

    if (!hasAccess) {
      return res.status(403).json({ error: 'Purchase does not include Negotiation Simulator access' });
    }
  } catch (stripeError) {
